...
<div id = "margin">
    ...
    <div id = "mapscroll">
        ...
        <a id = "mapuserlink" href= "scrolleditor.php">
...
mczoom.on("panleft panright panup pandown tap press", function(ev) {
    ...
    if(hmode){
        mainmap.linkArray[0].style.height = (Math.abs(mainmap.array[0].h*mainmap.w + 2*ev.deltaX)).toString() +  "px";
    }
    else{
        mainmap.linkArray[0].style.width = (Math.abs(mainmap.array[0].w*mainmap.w + 2*ev.deltaX)).toString() +  "px";
        mainmap.linkArray[0].style.height = (Math.abs(mainmap.array[0].h*mainmap.w + 2*ev.deltaX)).toString() +  "px";
        mainmap.linkArray[0].style.fontSize = (0.1*Math.abs(mainmap.array[0].w*mainmap.w + 2*ev.deltaX)).toString() + "px";
    }
...
document.getElementById("zoomoutbutton").onclick = function() {
    mainmap.array[0].w /= 1.25;
    if(hmode){
        mainmap.linkArray[mainmap.linkindex].style.height = (Math.abs(mainmap.array[mainmap.linkindex].h*mainmap.w)).toString() +  "px";
    }
    else{
        mainmap.linkArray[0].style.width = (Math.abs(mainmap.array[0].w*mainmap.w)).toString() +  "px";
        mainmap.linkArray[0].style.fontSize = (0.1*Math.abs(mainmap.array[0].w*mainmap.w)).toString() + "px";   
    }
}
...
if (this.readyState == 4 && this.status == 200) {
    ...
    //maps in the selector for the editor
